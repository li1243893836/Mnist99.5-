...
w0 = weight_variable([1024,10]) #FC 128 inputs,10 outputs
w4 = weight_variable([256*2*2,1024]) # FC 128 * 4 * 4 inputs, 625 outputs
...
#初始化第四个卷积层的权值和偏置
W_conv4=weight_variable([3,3,128,256]) #3*3的采样窗口，128个卷积核从64个平面抽取特征
b_conv4=bias_variable([256])
#把第二个池化层结果和权值向量进行卷积，再加上偏置值，然后应用于relu激活函数
h_conv4=tf.nn.relu(conv2d(h_pool3,W_conv4))
h_pool4=max_pool_2x2(h_conv4)
h_drop4=tf.nn.dropout(h_pool4,p_keep_conv) 

#合并所有的feature map
h_feature = tf.reshape(h_drop4,[-1,w4.get_shape().as_list()[0]])
